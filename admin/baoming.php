...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" style="border-left:1px solid #B7E6B0;border-right:1px solid #B7E6B0;">
  <tr> 
    <td height="30" background="images/main_title_bg.jpg">&nbsp;&nbsp;您的当前位置：<a href="#">系统首页</a> > 在线报名</td>
  </tr>
  <tr>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#BCE8B5">
            <tr>
              ...
              <td width="399" align="center" background="images/list_title_bg.jpg">报名时间</td>
              <td width="348" align="center" background="images/list_title_bg.jpg">所报专业</td>
              <td width="50" align="center" background="images/list_title_bg.jpg">操作</td>
            </tr>
<?php
$countSql="SELECT id FROM cnly_baoming";//sql语句
$KeyWord = "";
$pageSize="20"; //每页显示数
...
$sql = "SELECT * FROM cnly_baoming ORDER BY add_time DESC LIMIT $rsStart, $pageSize";
$res = $db->query($sql);
$i = 1;
...
?>            
            <tr <?=$style?>>
              ...
              <td align="center"><?=$row['classobj']?></td>
              <td align="center"><table width="50" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center"><span onClick="sAlertopen('550','480','bmup.php?id=<?=$row['id']?>&act=look');" style="font-size:12px; font-weight:normal; color:#FFFFFF; cursor:hand"><img src="images/icon_view.gif" width="16" height="16"  border="0"></span></td>
...
$url = "baoming.php?curPage=".$curPage."&".$urlPara."";
if($act == 'del'){
	$id = !empty($_GET['id']) ? intval($_GET['id']) : '';
	$sql = "DELETE FROM cnly_baoming WHERE id='$id'";
	$db->query($sql);
	urlMsg('删除成功',$url);
}
//全部删除
if($act=='delAll'){
	...
	if(!empty($bh)){
		foreach($bh as $key =>$value){
			echo $sql = "DELETE FROM cnly_baoming WHERE id='$value'";
			$db->query($sql);
		}
		urlMsg('删除成功',$url);
	}
